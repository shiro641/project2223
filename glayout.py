...
def drawRect(path, rects):
    ...
    colors = {'button': (255, 102, 0), 'heading': (255, 153, 204), 'link': (153, 153, 255), 'label': (102, 255, 255),
              'text': (51, 204, 51), 'image': (204, 0, 255), 'list': (153, 51, 0), 'searchInput': (0, 0, 102), 'dog': (204, 255, 255)}
    for rect in rects:
...
if __name__ == '__main__':
    # test0:Website Screenshots.v1-raw.voc/train/4shared_com_png.rf.7da0b596a12c1c1885dbdbc808d9c846
    # test1:Website Screenshots.v1-raw.voc/train/aarp_org_png.rf.08d458dd63a9db42109f55877c6ba82c
    # test2:biaozhu/pascal_voc_xml/screen_2000
    path = '../Website Screenshots.v1-raw.voc/train/4shared_com_png.rf.7da0b596a12c1c1885dbdbc808d9c846'
    rects = getRect('../input/preview.txt')
    drawRect('../input/tianmao.png', rects)
    diviedTimes = 1
    id = 0
    ...
    fo.write('</div>\n</template>\n')
    fo.write('<style scoped>\n')
    for st in style:
        fo.write('.'+st['name']+'{\n')
        for key in st['content']:
            # fo.write('{0}:{1}'.format(key, st['content'][key]))
            if key == 'width' or key == 'height' or key == 'margin-left' or key == 'margin-top':
                fo.write('{0}:{1}'.format(key, int(int(st['content'][key]))))
                fo.write('px;\n')
            else:
                fo.write('{0}:{1}'.format(key, st['content'][key]))
                fo.write(';\n')
